...
if ahz<arh/100: #solve differently if mode enters the horizon much before reheating
    ...
    delta0=[1,-2,k**2/2/astart/Ht(astart),-2/(1+w(astart)),k**2/2/astart/Ht(astart),-2,k**2/2/astart/Ht(astart)]
    delta=solve_ivp(lambda a,y: ddel(a,y,k),[astart,100*ahz],delta0,method='BDF',dense_output=False,atol=1e-8,rtol=1e-7)# Complete perturbation equations solving till 100ahz
    if ahz>np.sqrt(adom/arh*c2s(afz))*afz:
        deltasim0=[delta.y[0,-1],delta.y[1,-1],delta.y[2,-1],delta.y[3,-1],delta.y[4,-1]]#initial condition  deep in subhorizon
        deltasim=solve_ivp(lambda a,y: ddelsim(a,y,k),[delta.t[-1],5*arh],deltasim0,method='BDF',dense_output=False,atol=1e-6,rtol=1e-5)#ignore radiation perturbations completely
        ...
        deltasupersim0=[delta.y[3,-1],delta.y[4,-1]]#initial condition deep in subhorizon for just DM
        deltasupersim=solve_ivp(lambda a,y: ddel_super_sim(a,y,k),[delta.t[-1],5*arh],deltasupersim0,method='BDF',dense_output=False,atol=1e-6,rtol=1e-5)#ignore radiation, cannibal and metric perturbations completely
        A=deltasupersim.t[-1]*(deltasupersim.y[0,-1]-deltasupersim.y[0,-2])/(deltasupersim.t[-1]-deltasupersim.t[-2]) #Find A and B for dm density evolving as \delta=A*log(B*a) after reheating
        B=1/(deltasupersim.t[-1])*np.exp(deltasupersim.y[0,-1]/A)#finding B
        deltadm_post_rh=A*np.log(B*np.arange(deltasupersim.t[-1],10*arh,arh))# finding \delta_DM from arh to 1000arh using \delta=A*log(B*a)
        delta_dm=np.append(np.append(delta.y[3,:],deltasupersim.y[0,:]),deltadm_post_rh[:])# joinging the post reheating DM density evolution with pre-reheating
        a_full=np.append(np.append(delta.t[:],deltasupersim.t[:]),np.arange(deltasupersim.t[-1],10*arh,arh))
        delta_r=delta.y[5,:]
        a_delta_r=delta.t[:]
        delta_can=delta.y[1,:]
        a_delta_can=delta.t[:]
    ...
    delta0=[1,-2,k**2/2,-2/(1+w(astart)),k**2/2,-2,k**2/2]
    delta=solve_ivp(lambda a,y: ddel(a,y,k),[astart,5*arh],delta0,method='RK45',dense_output=False,atol=1e-7,rtol=1e-7)# Complete perturbation equations
    delta_post_rh0=[delta.y[0,-1],delta.y[3,-1],delta.y[4,-1],delta.y[5,-1],delta.y[6,-1]]
...
if ahz<arh/100: #solve differently if mode enters the horizon much before reheating
    ...
    delta0=[1,-2,k**2/2/astart/Ht(astart),-2/(1+w(astart)),k**2/2/astart/Ht(astart),-2,k**2/2/astart/Ht(astart)]
    delta=solve_ivp(lambda a,y: ddel(a,y,k),[astart,100*ahz],delta0,method='BDF',dense_output=False,atol=1e-8,rtol=1e-7)# Complete perturbation equations solving till 100ahz
    if ahz>np.sqrt(adom/arh*c2s(afz))*afz: #mode eventually escapes Jeans length
        deltasim0=[delta.y[0,-1],delta.y[1,-1],delta.y[2,-1],delta.y[3,-1],delta.y[4,-1]]#initial condition  deep in subhorizon
        deltasim=solve_ivp(lambda a,y: ddelsim(a,y,k),[delta.t[-1],5*arh],deltasim0,method='BDF',dense_output=False,atol=1e-6,rtol=1e-5)#ignore radiation perturbations completely
        A=deltasim.t[-1]*(deltasim.y[3,-1]-deltasim.y[3,-2])/(deltasim.t[-1]-deltasim.t[-2]) #Find A and B for dm density evolving as \delta=A*log(B*a) after reheating
        B=1/(deltasim.t[-1])*np.exp(deltasim.y[3,-1]/A)#finding B
...
plt.text(aplt[-1]/6,k1**-1*2,r"$k_1^{-1}$")
plt.loglog([aplt[0],aplt[-1]],[k2**-1,k2**-1],'--',color='black')  
plt.text(aplt[-1]/6,k2**-1*2,r"$k_{pk}^{-1}$")
plt.loglog([aplt[0],aplt[-1]],[k3**-1,k3**-1],'--',color='blue')  
plt.text(aplt[-1]/6,k3**-1/5,r"$k_2^{-1}$")
...
plt.text(minx*1.2,1.6*krh**-1,r"$k_{rh}^{-1}$")
plt.loglog([minx,maxx],[kdom**-1,kdom**-1],'--',color='purple')
plt.text(minx*1.2,1.6*kdom**-1,r"$k_{dom}^{-1}$")
plt.loglog([minx,maxx],[kpk**-1,kpk**-1],'--k')  
plt.text(minx*1.2,1.8*kpk**-1,r"$k_{pk}^{-1}$")
plt.xlim([minx,maxx])
